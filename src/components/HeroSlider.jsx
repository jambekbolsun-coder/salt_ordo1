      ...
      <div className="hero-slider__viewport" onPointerDown={pointerDown} onPointerUp={pointerUp}>
        {slides.map((slide, index) => {
          ...
          const action = index === 0
            ...
            : index === 1
              ? { href:'#contacts' }
              : { href:settings.instagram, external:true }
          return (
            <article className={`hero-slide ${active === index ? 'is-active' : ''}`} key={slide.title} aria-hidden={active !== index}>
              ...
              <div className="container hero-slide__content">
                <span className="hero-slide__eyebrow"><Icon/>{slide.eyebrow}</span>
                <h1>{slide.title}</h1>
                <p>{slide.text}</p>
                {action.to
                  ? <Link className="btn btn--primary" to={action.to}>{slide.cta}<ArrowRight/></Link>
                  : <a className="btn btn--primary" href={action.href} target={action.external ? '_blank' : undefined} rel={action.external ? 'noreferrer' : undefined}>{index === 2 && <Instagram/>}{slide.cta}<ArrowRight/></a>}
              </div>
            </article>
          )
        })}
        <button className="hero-slider__arrow is-prev" type="button" onClick={() => change(-1)} aria-label="Previous slide"><ChevronLeft/></button>
        <button className="hero-slider__arrow is-next" type="button" onClick={() => change(1)} aria-label="Next slide"><ChevronRight/></button>
