...
import { getAnalyticsData, listCategories, listChatbotFaqs, listLeads, listProducts } from '../../lib/api'
...
export default function Dashboard() {
  ...
  const [data, setData] = useState({ products:[], categories:[], faqs:[], leads:[], analytics:{events:[],quizzes:[],leads:[]} })
  ...
  useEffect(() => {
    ...
    Promise.all(jobs)
      .then(([products,categories,faqs,leads,analytics]) => active && setData({ products,categories,faqs,leads,analytics }))
      .catch((err) => active && setError(err.message))
      .finally(() => active && setLoading(false))
    return () => { active = false }
  }, [catalogAccess, crmAccess])
  ...
  const metrics = useMemo(() => {
    ...
    return items
  }, [data, catalogAccess, crmAccess])

  return <>
    <AdminPageHeader eyebrow="Salt Ordo · Dashboard" title="Управление сайтом" text="Каталог, заявки, аналитика и ответы помощника — без лишних разделов." actions={catalogAccess ? <Link className="btn btn--primary" to="/admin/products/new">Добавить товар</Link> : null}/>
    {error && <div className="notice notice--error">{error}</div>}
    {!catalogAccess && !crmAccess && <div className="notice">Для этой роли сейчас нет отдельных разделов управления.</div>}
